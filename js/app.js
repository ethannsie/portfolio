...
/* ---------- notes gallery ----------
   ...
   scripts/build-notes.js from whatever PDFs sit in assets/notes/. */
function renderNotes() {
  ...
  function card(n, i) {
    const meta = [n.pages ? `${n.pages} page${n.pages === 1 ? "" : "s"}` : null, n.size]
      .filter(Boolean).join(" · ");
    return `
      <a class="card note-card" href="${esc(n.file)}" target="_blank" rel="noopener" style="animation-delay:${Math.min(i, 8) * 45}ms">
        <div class="note-thumb-wrap">
          <img class="note-thumb" src="${esc(n.thumb)}" alt="First page of ${esc(n.title)}" loading="lazy">
          <span class="note-badge mono">PDF</span>
        </div>
        <div class="card-body">
          <h2 class="card-title">${esc(n.title)}</h2>
          <div class="card-meta mono"><span>${esc(meta)}</span></div>
        </div>
      </a>`;
  }
  ...
  grid.innerHTML = list.length
    ? list.map(card).join("")
    : `<p class="empty">No notes yet — drop a PDF into <span class="mono">assets/notes/</span> and commit; it shows up here automatically.</p>`;
